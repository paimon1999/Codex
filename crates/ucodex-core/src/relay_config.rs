...
pub(crate) fn table_mut_or_insert<'a>(doc: &'a mut DocumentMut, key: &str) -> anyhow::Result<&'a mut Table> {
...
pub(crate) fn table_mut_if_exists<'a>(doc: &'a mut DocumentMut, key: &str) -> Option<&'a mut Table> {
...
pub async fn test_relay_profile(
    ...
    let client = crate::http_client::proxied_client("Ucodex/RelayTest")?;
...
pub(crate) fn parse_toml_document(contents: &str) -> anyhow::Result<DocumentMut> {
...
pub(crate) fn normalize_duplicate_toml_text(contents: &str) -> String {
...
fn create_live_backup(
    ...
    let backup_dir = home
        .join("backups")
        .join(format!("ucodex-live-{}", timestamp_millis()));
...
pub(crate) fn ensure_trailing_newline(mut contents: String) -> String {
