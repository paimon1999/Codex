...
pub const SILENT_NAME: &str = "Ucodex";
pub const MANAGER_NAME: &str = "Ucodex 管理工具";
pub const SILENT_BINARY: &str = "ucodex";
pub const MANAGER_BINARY: &str = "ucodex-manager";
...
pub fn shortcut_names() -> (&'static str, &'static str) {
    ("Ucodex.lnk", "Ucodex 管理工具.lnk")
}

pub fn app_bundle_names() -> (&'static str, &'static str) {
    ("Ucodex.app", "Ucodex 管理工具.app")
}
...
fn platform_install(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持安装 Ucodex 入口")
    }
}
...
fn platform_uninstall(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持卸载 Ucodex 入口")
    }
}
