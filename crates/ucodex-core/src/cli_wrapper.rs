...
const CLI_HOME_DIR: &str = ".ucodex-cli";
...
pub fn install_cli_wrapper_to(
    ...
    std::fs::create_dir_all(codex_home)
        .with_context(|| format!("failed to create Ucodex CLI home {}", codex_home.display()))?;
...
fn compile_wrapper(source_path: &Path, wrapper_path: &Path) -> anyhow::Result<()> {
    let csc =
        find_csc().ok_or_else(|| anyhow::anyhow!("未找到 csc.exe，无法编译 Ucodex wrapper"))?;
