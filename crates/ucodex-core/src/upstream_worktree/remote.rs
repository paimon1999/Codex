...
fn codex_global_state_path_for_ssh() -> std::path::PathBuf {
...
fn resolve_ssh_target_for_host_id(
    ...
    if host_id.is_empty() {
        return Err("Remote host id is required".into());
    }
    let path = state_path
        .map(std::path::PathBuf::from)
        .unwrap_or_else(codex_global_state_path_for_ssh);
    let data = std::fs::read_to_string(&path)
        .map_err(|error| format!("Cannot read Codex remote connection state: {error}"))?;
    let state: serde_json::Value = serde_json::from_str(&data)
        .map_err(|error| format!("Cannot parse Codex remote connection state: {error}"))?;
...
fn resolve_ssh_target_from_global_state(
    ...
    for connection in connections {
        ...
        let conn_host_id = connection.get("hostId")
            .and_then(|v| v.as_str())
            .unwrap_or_default()
            .trim();
        if conn_host_id != host_id {
            continue;
        }
        let user = connection.get("sshUser")
            .and_then(|v| v.as_str())
            .unwrap_or_default()
            .trim()
            .to_string();
        let host = connection.get("sshHost")
            .and_then(|v| v.as_str())
            .unwrap_or_default()
            .trim()
            .to_string();
        let port = connection.get("sshPort")
            .and_then(|v| v.as_u64())
            .map(|p| p as u16);
        if host.is_empty() {
            return Err("Cannot resolve remote SSH host for this file".into());
        }
        return Ok(SshTarget { user, host, port });
    }
    Err("Cannot resolve remote SSH host for this file".into())
}
...
fn remote_git(project: &UpstreamRemoteProject, args: &[&str]) -> UpstreamWorktreeResult<GitOutput> {
    let target = resolve_ssh_target_for_host_id(&project.host_id, None).map_err(|error| {
        UpstreamWorktreeError::new(UpstreamWorktreeCode::GitMissing, error)
    })?;
