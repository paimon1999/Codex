...
use ucodex_core::settings::{RelayContextSelection, RelayMode, RelayProfile, RelayProtocol};
...
fn apply_chat_protocol_relay_points_codex_to_local_responses_proxy() {
    ...
    let result = ucodex_core::relay_config::apply_relay_config_to_home_with_protocol(
        temp.path(),
...
"#,
    ...
    assert!(backup_path.contains("ucodex-live-"));
    assert_eq!(
...
fn apply_relay_files_with_context_selection_writes_all_global_context() {
    ...
    ucodex_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "custom"
[model_providers.custom]
name = "custom"
...
fn apply_relay_files_with_context_rejects_invalid_context_token_values() {
    ...
    let error = ucodex_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "custom""#,
        r#"{"OPENAI_API_KEY":"sk-new"}"#,
        "",
...
"#,
    ...
    assert!(
        result
            .backup_path
            .as_ref()
            .is_some_and(|path| path.contains("ucodex-live-"))
    );
    assert!(updated.contains(r#"model = "gpt-5""#));
