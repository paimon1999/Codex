use ucodex_core::assets;
use ucodex_core::bridge::{self, BRIDGE_BINDING_NAME};
use ucodex_core::cdp::{CdpTarget, list_targets, pick_page_target};
...
fn injection_script_prefixes_helper_url_and_version() {
    ...
    assert!(script.contains("window.__UCODEX_VERSION__"));
    assert!(script.contains(ucodex_core::version::VERSION));
    assert!(script.contains("https://discord.gg/y96kX7A76v"));
    assert!(script.contains("data-ucodex-discord"));
}
...
fn injection_script_marks_diagnostic_build_and_reports_script_loaded() {
    ...
    assert!(script.contains("window.__UCODEX_BUILD__"));
    assert!(script.contains(ucodex_core::assets::DIAGNOSTIC_BUILD_ID));
    assert!(script.contains("script_loaded"));
    assert!(script.contains("data-ucodex-build"));
}
...
fn injection_script_menu_exposes_three_independent_plugin_switches() {
    ...
    assert!(script.contains("data-ucodex-setting=\"pluginMarketplaceUnlock\""));
    assert!(script.contains("强制解锁入口"));
    assert!(script.contains("data-ucodex-setting=\"pluginEntryUnlock\""));
    assert!(script.contains("特殊插件强制安装"));
    assert!(script.contains("data-ucodex-setting=\"forcePluginInstall\""));
...
fn injection_script_keeps_bundled_marketplace_name_for_default_filter() {
    ...
    assert!(!script.contains("if (name === \"openai-bundled\") return \"ucodex-openai-bundled\""));
    assert!(script.contains("if (name === \"openai-bundled\" || name === \"ucodex-openai-bundled\") return \"OpenAI插件1(Codex++)\""));
}
...
fn injection_script_expands_api_key_plugin_marketplace_requests() {
    ...
    assert!(script.contains("if (name === \"openai-bundled\") return \"\""));
    assert!(script.contains("if (name === \"openai-curated\") return \"ucodex-openai-curated\""));
    assert!(script.contains("if (name === \"openai-primary-runtime\") return \"ucodex-openai-primary-runtime\""));
...
fn injection_script_exposes_conversation_view_width_control() {
    ...
    assert!(script.contains("data-ucodex-conversation-view-width"));
...
fn injection_script_exposes_fast_service_tier_control() {
    ...
    assert!(script.contains("data-ucodex-setting=\"serviceTierControls\""));
...
fn manager_ui_exposes_pure_api_relay_mode_button() {
    let repo = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("core crate should live under crates/ucodex-core");
    let source = std::fs::read_to_string(repo.join("apps/ucodex-manager/src/App.tsx")).unwrap();
    let commands =
        std::fs::read_to_string(repo.join("apps/ucodex-manager/src-tauri/src/lib.rs")).unwrap();
...
async fn install_bridge_routes_binding_while_waiting_for_command_response() {
    ...
    let log_path = temp.path().join("ucodex.log");
    ucodex_core::diagnostic_log::set_diagnostic_log_path_for_tests(Some(log_path.clone()));
    ...
    ucodex_core::diagnostic_log::set_diagnostic_log_path_for_tests(None);
}
