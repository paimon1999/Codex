...
fn windows_entrypoint_plan_contains_silent_and_manager_entrypoints() {
    let options = InstallOptions {
        ...
        launcher_path: Some("C:/Tools/ucodex.exe".into()),
        manager_path: Some("C:/Tools/ucodex-manager.exe".into()),
    ...
    assert_eq!(plan.launcher_path, "C:/Tools/ucodex.exe");
    assert_eq!(plan.manager_path, "C:/Tools/ucodex-manager.exe");
    assert_eq!(plan.silent_icon_path, "C:/Tools/ucodex.exe");
    assert_eq!(
        plan.manager_icon_path,
        "C:/Tools/ucodex-manager.exe"
    );
...
fn macos_bundle_metadata_contains_silent_and_manager_apps() {
    let options = InstallOptions {
        ...
        launcher_path: Some("/opt/Codex++/ucodex".into()),
        manager_path: Some("/opt/Codex++/ucodex-manager".into()),
        remove_owned_data: false,
    };
    ...
    assert!(silent.launch_script.contains("ucodex"));
    assert!(manager.launch_script.contains("ucodex-manager"));
}
...
fn companion_binary_path_resolves_macos_silent_app_next_to_manager_app() {
    ...
    assert_ne!(
        ...
        std::path::PathBuf::from(
            "/Applications/Codex++ 管理工具.app/Contents/MacOS/ucodex"
        )
    );
}
