...
!define MUI_ICON "${ROOT}\apps\ucodex-manager\src-tauri\icons\icon.ico"
!define MUI_UNICON "${ROOT}\apps\ucodex-manager\src-tauri\icons\icon.ico"
...
Section "Install"
  ...
  nsExec::ExecToLog 'taskkill /IM ucodex.exe /F'
  Pop $0
  nsExec::ExecToLog 'taskkill /IM ucodex-manager.exe /F'
  Pop $0

  File "${ROOT}\dist\windows\app\ucodex.exe"
  File "${ROOT}\dist\windows\app\ucodex-manager.exe"

  Delete "$DESKTOP\Codex++ 绠＄悊宸ュ叿.lnk"
  Delete "$SMPROGRAMS\Codex++\Codex++ 绠＄悊宸ュ叿.lnk"

  CreateShortcut "$DESKTOP\Codex++.lnk" "$INSTDIR\ucodex.exe" "" "$INSTDIR\ucodex.exe"
  CreateShortcut "$DESKTOP\Codex++ 管理工具.lnk" "$INSTDIR\ucodex-manager.exe" "" "$INSTDIR\ucodex-manager.exe"
  CreateDirectory "$SMPROGRAMS\Codex++"
  CreateShortcut "$SMPROGRAMS\Codex++\Codex++.lnk" "$INSTDIR\ucodex.exe" "" "$INSTDIR\ucodex.exe"
  CreateShortcut "$SMPROGRAMS\Codex++\Codex++ 管理工具.lnk" "$INSTDIR\ucodex-manager.exe" "" "$INSTDIR\ucodex-manager.exe"
  CreateShortcut "$SMPROGRAMS\Codex++\卸载 Codex++.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\ucodex-manager.exe"
  ...
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\Codex++" "DisplayIcon" "$INSTDIR\ucodex-manager.exe"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\ucodex.exe"
  Delete "$INSTDIR\ucodex-manager.exe"
