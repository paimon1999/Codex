...
DMG="$DIST/Ucodex-${VERSION}-macos-${ARCH}.dmg"
ICON_SOURCE="$ROOT/apps/ucodex-manager/src-tauri/icons/icon.png"
ICON_NAME="ucodex.icns"
...
prepare_icon() {
  local iconset="$DIST/ucodex.iconset"
  rm -rf "$iconset"
  mkdir -p "$iconset"
...
prepare_icon
create_app "Ucodex" "Ucodex" "$BINARY_DIR/ucodex" "com.bigpizzav3.codexplusplus" "true"
create_app "Ucodex 管理工具" "UcodexManager" "$BINARY_DIR/ucodex-manager" "com.bigpizzav3.codexplusplus.manager" "false"
ln -s /Applications "$STAGE/Applications"

sign_app "$STAGE/Ucodex.app"
sign_app "$STAGE/Ucodex 管理工具.app"

verify_app "$STAGE/Ucodex.app"
verify_app "$STAGE/Ucodex 管理工具.app"

hdiutil create -volname "Ucodex" -srcfolder "$STAGE" -ov -format UDZO "$DMG"
echo "$DMG"
