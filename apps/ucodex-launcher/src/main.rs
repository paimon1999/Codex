...
use ucodex_core::launcher::{
    DefaultLaunchHooks, LaunchHooks, LaunchOptions, launch_and_inject_with_hooks,
};
use ucodex_core::models::{DeleteResult, ExportResult, SessionRef};
use ucodex_core::routes::{BridgeContext, BridgeDataService, BridgeRuntimeService};
...
fn acquire_single_instance_guard(
    debug_port: u16,
) -> anyhow::Result<Option<ucodex_core::ports::LoopbackPortGuard>> {
        ...
        Err(error) => Err(error)
            .with_context(|| {
                format!(
                    ...
                    ucodex_core::ports::LAUNCHER_GUARD_PORT
                )
            })
            .map(Some),
    }
...
fn try_acquire_single_instance_guard() -> std::io::Result<ucodex_core::ports::LoopbackPortGuard>
{
    ucodex_core::ports::acquire_resilient_loopback_port_guard(
        ucodex_core::ports::LAUNCHER_GUARD_PORT,
    )
}
...
fn log_launcher_guard_fallback(fallback_lock_path: &Path) {
    let _ = ucodex_core::diagnostic_log::append_diagnostic_log(
        ...
        json!({
            "requested_guard_port": ucodex_core::ports::LAUNCHER_GUARD_PORT,
...
fn should_recover_stale_launcher(debug_port: u16) -> bool {
    let has_codex_process = !ucodex_core::watcher::find_codex_processes().is_empty();
    let cdp_listening = ucodex_core::watcher::cdp_listening(debug_port);
    let recover =
        ucodex_core::watcher::should_recover_stale_launcher(has_codex_process, cdp_listening);
    let _ = ucodex_core::diagnostic_log::append_diagnostic_log(
...
async fn activate_existing_codex_app(options: &LaunchOptions) -> anyhow::Result<()> {
    ...
    let process_ids = ucodex_core::watcher::find_codex_processes();
    let mut activated = false;
    #[cfg(windows)]
    {
        for process_id in &process_ids {
            if ucodex_core::windows_activate_process_window(*process_id) {
                activated = true;
                break;
            }
        }
...
fn log_launcher_already_running(debug_port: u16) {
    let _ = ucodex_core::diagnostic_log::append_diagnostic_log(
        ...
        json!({
            "guard_port": ucodex_core::ports::LAUNCHER_GUARD_PORT,
            "debug_port": debug_port
        }),
    );
}
...
async fn notify_manager_when_update_available() -> anyhow::Result<bool> {
    let update =
        ucodex_core::update::check_for_update(ucodex_core::version::VERSION).await?;
...
fn open_manager_with_update_prompt() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        command.creation_flags(ucodex_core::windows_create_no_window());
    }
    command
        .spawn()
        .map(|_| ())
...
impl LaunchHooks for LauncherHooks {
    fn resolve_app_dir(
        ...
        settings: &ucodex_core::settings::BackendSettings,
    ...
    async fn load_settings(&self) -> anyhow::Result<ucodex_core::settings::BackendSettings> {
        self.core.load_settings().await
    }

    async fn run_provider_sync(&self) -> anyhow::Result<()> {
        let _ = tokio::task::spawn_blocking(|| ucodex_data::run_provider_sync(None))
            .await
    ...
    async fn launch_codex(
        ...
        debug_port: u16,
        extra_args: &[String],
    ) -> anyhow::Result<ucodex_core::launcher::CodexLaunch> {
        self.core
    ...
    async fn wait_for_codex_exit(
        &self,
        launch: &ucodex_core::launcher::CodexLaunch,
    ) -> anyhow::Result<()> {
    ...
    async fn terminate_codex(&self, launch: &ucodex_core::launcher::CodexLaunch) {
...
impl Default for LauncherDataService {
    fn default() -> Self {
        Self {
            ...
            backup_dir: ucodex_core::paths::default_app_state_dir().join("backups"),
        }
    }
}
...
impl BridgeDataService for LauncherDataService {
    ...
    async fn export_markdown(&self, session: SessionRef) -> anyhow::Result<ExportResult> {
        let export_service =
            ucodex_data::MarkdownExportService::new(Some(self.db_path.clone()));
...
impl LauncherDataService {
    fn storage_adapter(&self) -> ucodex_data::SQLiteStorageAdapter {
        ucodex_data::SQLiteStorageAdapter::new(
            self.db_path.clone(),
            ucodex_data::BackupStore::new(self.backup_dir.clone()),
        )
    }
}
...
impl BridgeRuntimeService for LauncherRuntimeService {
    ...
    async fn reload_user_scripts(&self) -> anyhow::Result<Value> {
        ...
        if let Some(websocket_url) = websocket_url.filter(|_| !bundle.trim().is_empty()) {
            ucodex_core::bridge::evaluate_script(&websocket_url, &bundle).await?;
        }
        self.user_scripts.inventory()
    }
    ...
    async fn open_devtools(&self) -> anyhow::Result<Value> {
        ...
        let targets = ucodex_core::cdp::list_targets(debug_port).await?;
        let target = ucodex_core::cdp::pick_page_target(&targets)?;
        let url = ucodex_core::routes::devtools_url(debug_port, &target.id);
        open_url(&url)?;
        Ok(json!({
            "status": "ok",
            "target_id": target.id,
    ...
    async fn open_manager(&self) -> anyhow::Result<Value> {
        ...
        #[cfg(windows)]
        {
            std::process::Command::new(&manager_path)
                .creation_flags(ucodex_core::windows_create_no_window())
                .spawn()
                .map_err(|error| anyhow::anyhow!("启动管理工具失败：{error}"))?;
        }
        #[cfg(not(windows))]
    ...
    async fn backend_status(&self) -> anyhow::Result<Value> {
        Ok(
            json!({"status": "ok", "message": "后端已连接", "version": ucodex_core::version::VERSION}),
        )
    }
...
async fn try_inject_with_context(
    ...
    let script = ucodex_core::assets::injection_script(helper_port);
    ...
    ucodex_core::bridge::install_bridge(
        websocket_url,
        ucodex_core::bridge::BRIDGE_BINDING_NAME,
        Arc::new(move |path, payload| {
            let ctx = ctx.clone();
            Box::pin(async move {
                Ok(ucodex_core::routes::handle_bridge_request(ctx, &path, payload).await)
            })
        }),
...
fn manager_exe_path() -> PathBuf {
    ...
    dir.join(format!(
        "{}{}",
        ucodex_core::install::MANAGER_BINARY,
        suffix
    ))
}
    ...
    fn manager_update_prompt_uses_sidecar_manager_binary_name() {
        ...
        assert!(
            path.file_name()
                ...
                .is_some_and(|name| name.contains(ucodex_core::install::MANAGER_BINARY))
        );
    }
}
